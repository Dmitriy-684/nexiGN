...
## Программная часть решения
Точкой входа в программу является класс App с единственным методом main, в котором из spring контекста достаются бины DataBase, GenerateCDRFile, GenerateUDRFiles. В цикле с параметром происходит генерация CDR файла за определённый месяц (CDR файл помещается в ./cdrFiles), после чего информация из данного CDR записывется в базу данных h2, далее на основе CDR файлов в директории ./reports генерируются отчёты, после чего сервис завершает свою работу
